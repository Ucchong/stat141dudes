...
#HIGH BP
barplot(table(train_data$HighBP), col = c("tomato2", "darkgreen"), beside = T, names.arg = c("No High Blood Pressure", "High Blood Pressure"), main = "High Blood Pressure Distribution", ylab = "Frequency", sub="Indicates if the person has been told by a health professional \nthat they have High Blood Pressure.")
#HIGH CHOL
barplot(table(train_data$HighChol), col = c("tomato2", "darkgreen"), beside = T, names.arg = c("No High Cholestrol", "High Cholestrol"), main = "High Cholestrol Distribution", ylab = "Frequency", sub="Indicates if the person has been told by a health professional that they have \nHigh Blood Cholesterol.")
# CholCheck
barplot(table(train_data$CholCheck), col = c("tomato2", "darkgreen"), beside = T, names.arg = c("No Cholesterol Check", "Yes Cholesterol Check"), main = "Cholesterol Check Distribution", ylab = "Frequency", sub = "Indicates if the person has their cholesterol levels checked \nwithin the last 5 years.")
#Smoker
barplot(table(train_data$Smoker), col = c("tomato2", "darkgreen"), beside = T, names.arg = c("No Smoker", "Smoker"), main = "Smoker Distribution", ylab = "Frequency", sub = "Indicates if the person has smoked at least 100 cigarettes.")
#Stroke
barplot(table(train_data$Stroke), col = c("tomato2", "darkgreen"), beside = T, names.arg = c("No Stroke History", "Stroke History"), main = "Stroke Distribution", ylab = "Frequency", sub = "Indicates if the person has a history of stroke.")
# Diabetes
barplot(table(train_data$Diabetes), col = c("tomato2", "darkgreen"), beside = T, names.arg = c("No Diabetes", "Pre-Diabetes", "Diabetes"), main = "Diabetes Distribution", ylab = "Frequency")
# HvyAlcoholConsump
barplot(table(train_data$HvyAlcoholConsump), col = c("tomato2", "darkgreen"), beside = T, names.arg = c("No Heavy Alcohol Consumption", "Heavy Alcohol Consumption"), main = "Heavy Alcohol Consumption Distribution", ylab = "Frequency", sub = "Indicates if the person has more than 14 drinks per week.")
# NoDocbcCost
barplot(table(train_data$NoDocbcCost), col = c("tomato2", "darkgreen"), beside = T, names.arg = c("Did not experience", "No Doctor Visit because of Cost"), main = "No Doctor Because of The Cost", ylab = "Frequency", sub= "Indicates if the person wanted to visit a doctor within the past 1 year \nbut couldn’t, due to cost.")
# DiffWalk
barplot(table(train_data$DiffWalk), col = c("tomato2", "darkgreen"), beside = T, names.arg = c("No Difficulty Walking", "Difficulty Walking"), main = "Difficulty Walking Distribution", ylab = "Frequency", sub = "Indicates if the person has difficulty while walking or climbing stairs.")
# Sex
barplot(table(train_data$Sex), col = c("tomato2", "darkgreen"), beside = T, names.arg = c("Female", "Male"), main = "Sex Distribution", ylab = "Frequency")
# Age
color_gradient <- colorRampPalette(c("tomato2", "darkgreen"))
...
boxplot(list(data0$Age, data1$Age),  col = c("tomato2", "darkgreen"), names = c("No Heart Attack", "Heart Attack"), xlab = "Heart Attack Status", ylab = "Age", main = "Boxplot of Age by Heart Attack Status") #outliers for young age and heart attacks
